...
# --- Nouvelle Inscription arrivé à l'ADMIN ---
...
# --- Dit à un User que son compte est validé ---
...
# --- Informé à l'autorité Locale qu'il y a un nouveau Incident doit validé  ---
def send_new_incident_to_authorities(authority_emails: List[str], incident_title: str, incident_id: int, user_name: str, incident_description: str):
    """Notifie toutes les autorités locales d'un nouvel incident."""
    subject = f"[GIF Mada] Nouvel Incident Signalé: #{incident_id} - {incident_title}"
    body = (
        f"Bonjour,\n\n"
        f"Un nouvel incident a été signalé et requiert votre attention.\n"
        f"  - Incident ID: {incident_id}\n"
        f"  - Titre: {incident_title}\n"
        f"  - Signalé par: {user_name}\n"
        f"  - Description: {incident_description}\n\n"
        f"Veuillez vous connecter à votre tableau de bord pour le consulter, le valider et l'assigner.\n\n"
        f"L'équipe GIF Mada."
    )
    for email in authority_emails:
        send_email(email, subject, body)

# --- Dit à un Sécurité Urbaine Assigné qu'elle a de nouvelle mission ---
def send_assignment_to_security(agent_email: str, incident_title: str, incident_id: int, incident_description: str, incident_address: str):
    ...
    body = (
        ...
        f"  - Description: {incident_description}\n"
        f"  - Adresse: {incident_address or 'Voir sur la carte'}\n\n"
...
def send_panic_alert_notification(emails: List[str], incident_id: int, location: Dict):
    ...
    body = (
        ...
        f"Toutes les unités disponibles sont priées de consulter l'application pour les détails et de se rendre sur les lieux.\n\n"
...
def send_password_reset_email(user_email: str, reset_token: str):
    ...
    # IMPORTANT: L'URL doit pointer vers votre page frontend
